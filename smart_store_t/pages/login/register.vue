<template>
	<view class="forget">
		<cu-custom bgColor="bg-white" :isBack="true">
			<block slot="backText" ><navigator url="login"><text class="text-lg">注册</text></navigator></block>
			<block slot="content"></block>
		</cu-custom>
		
		<view class="content marin-top" style="margin-top: 100upx;">
			<form>
			<view class="cu-form-group ">
				<view class="title">账 户</view>
				<input placeholder="点击输入手机号码" name="input" v-model="phoneData"></input>
			</view>
			<view class="cu-form-group ">
				<view class="title">验证码</view>
				<input placeholder="点击输入收到的验证码" v-model="verCode" name="input"></input>
				<button class='cu-btn bg-blue shadow' v-show="!isRunCode" @click="getVerCode">{{verCodeText}}</button>
				<button class='cu-btn bg-blue shadow' v-show="isRunCode" >{{second}}s后获取</button>
			</view>
			<view class="cu-form-group ">
				
				<view class="title">密 码</view>
				<input   placeholder="8-16位数字组成的密码" name="input" type="password" v-model="passData"></input>
			</view>
			<view class="cu-form-group solid-bottom  margin-bottom">
				<view class="title">确认密码</view>
				<input placeholder="再次输入密码" name="input" type="password" 	v-model="passDataNext"></input>
			</view>
			</form>
			<wButton 
				text="注册"
		...
		<view class="rules">
			...
			<view class="rules-text" @click="chan">
				<radio class="radios" :checked="isChecked" />
				<span >同意</span>
	...
	export default {
		...
		methods: {
			...
			checkPhoneRules() {
				...
				if (this.phoneData && !regex.test(this.phoneData)) {
					uni.showToast({
					    icon: 'none',
						position: 'bottom',
					    title: '此电话号码无效'
			...
			checkPasswordRules() {
				...
				if (!this.passData) {
					...
					return false;
				} else if (this.passData && (this.passData.length < 8 || this.passData.length > 16)) {
					uni.showToast({
					    icon: 'none',
						position: 'bottom',
					    title: '请输入8-16位字母数字组合'
					});
					return false;
				} else if (this.passData && !regex.test(this.passData)) {
					uni.showToast({
					    icon: 'none',
						position: 'bottom',
					    title: '密码格式错误'
			...
			checkRePasswordRules() {
				if (!this.passData) {
					...
					return false;
				} else if (this.passData && this.passDataNext && this.passData !== this.passDataNext) {
					uni.showToast({
					    icon: 'none',
						position: 'bottom',
