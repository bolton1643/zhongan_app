	...
	<view class="login">
		<view class="content ">
			...
			<view class="header  text-center"  >
				<image :src="'/static/image/login/zhihuimengdian.png'"></image>
			</view>
			<!-- 主体表单 -->
			<view class="main"  >
				<view class="cu-form-group">
					<view class="title">账号</view>
					<input type="text" v-model="phoneData" placeholder="点击输入账号" name="input"></input>
				</view>
				<view class="cu-form-group " style="border-bottom: 0.5px solid #eee; margin-bottom: 50upx;">
					<view class="title">密码</view>
					<input v-model="passData" type="password" placeholder="点击输入密码"  name="input"></input>
				</view>
			<!-- 	<wInput
					v-model="phoneData"
					type="text"
			...
			<wButton
				...
				class="wbutton"
			></wButton>
			<!-- <wButton class="cu-btn-1 " :rotate="isRotate"  style="margin-top: -20upx;" @click.native="startLogin()">登 录</wButton> -->

			<view class="flex     justify-between">
				<view class="margin-left    text-sm padding" style="margin-left: 50upx;">
					<navigator url="forget" open-type="navigate"><text class="text-blue">忘记密码? </text></navigator>
				</view>
				<view class="margin-left    text-sm padding"  style="margin-right: 50upx;">
					<navigator url="register" open-type="navigate"><text class="text-black">没有账号，去注册</text></navigator>
				</view>
			</view>

			<view class="flex   padding " style="margin-top: 250upx; " >
				<view  class="qitadenglu">-- 其他登入方式 --</view>
			</view>

			<!-- 其他登录 -->
			<view class="other_login cuIcon" style="margin-top: 50upx;"  >
				<view class="login_icon">
					<view class="" @tap="login_alipay">
						<image :src="'/static/image/login/alipay.png'" style="width: 72upx;height: 72upx;"></image>
					</view>
				</view>
				<view class="login_icon">
					<view class="" @tap="login_weixin">
						<image :src="'/static/image/login/weixin.png'" style="width: 72upx;height: 72upx;"></image>
					</view>
				</view>
			<!-- 	<view class="login_icon">
...
<script>
	...
	export default {
		...
		methods: {
			...mapMutations(['login','WEBSOCKET_INIT']),
			isLogin(){
				//判断缓存中是否登录过，直接登录
				try {
					...
					if (value) {
						//有登录信息
						// _this.$store.dispatch("uerInfo",value); //存入状态
						uni.reLaunch({
							url: '../index/index',
						});
					}
				...
				this.$api.login(data).then(res => {
				   // 获得数据
					if (res.status === 200) {//登录成功后改变vuex的状态，并退出登录页面
						...
						_this.WEBSOCKET_INIT("ws://47.111.166.221:8080/api/tuser/websocket/" + this.phoneData)
						let loginData = res.result.userInfo
						loginData.token = res.result.token
						// uni.reLaunch({
						// 	url: '../index/index',
						// });
						_this.isRotate=false
						_this.toMain(loginData)
					}else {
						uni.showToast({
						    icon: 'none',
							position: 'bottom',
