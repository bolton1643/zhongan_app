			...
			<view class="cu-list menu-avatar bg-gray">
				<view class="cu-item margin-top margin-bottom listitem "  style="margin-left: 20upx; height: 250upx; padding-right: 0px;" v-for="(item, index) in installTaskList" :key="index" @click="gotoDetail(item)">
					<view class="content padding " style="left: 20upx; width: 100%;">
						<view class="text-black text-bold margin-top">{{item.shopName}}({{item.shopNo || '无' }})</view>
						<!-- <view class="text-grey text-xs margin-left fr " style=""  >{{ item.time }}</view></view> -->
						<view class="flex  p-xs margin-bottom-sm mb-sm">
							<!-- <view class="flex-sub   padding margin-xs radius text-center" > -->
							...
							<!-- </view> -->
							<view class="flex-treble   padding-top-sm margin-top-xs radius" >
								<view class="text-black text-bold listitem-text">地址：{{item.shopAddr || '无'}}</view>
								<view class="listitem-text-gay text-grey margin-top  " >负责人：{{item.manager || '无'}} {{item.managerPhone || '无'}}</view>
								<!-- <view class="listitem-text-gay text-grey margin-top  " >{{item.createTime}}</view> -->
							</view>
						</view>
					</view>
				</view>
...
export default {
	...
	methods: {
		repairLists() {
			...
			this.$api.combochangeList(params).then(res => {
			   // 获得数据 
		...
		gotoDetail(e){
			uni.navigateTo({
				url: '../indexgroup/packageDetail?id=' + e.id
			});
		}
	}
};
