<template name="deviceslist">
	<view>
		<cu-custom bgColor="bg-blue" :isBack="true" ><block slot="backText">返回</block>
			...
			<block slot="right"  ><text  @click="gotoAdd">新增</text>   </block>
		</cu-custom>
	 
		 
		<view  >
			<view class="cu-list menu" :class="[menuBorder?'sm-border':'',menuCard?'card-menu margin-top':'']">
				<view class="cu-item" :class="'arrow'" v-for="(item,index) in deviceListArr" :key="index"  @click="gotoDetail(item)">
					<view class="content">
						<text class="cuIcon-tagfill text-red  margin-right-xs"></text>
						<text class="text-grey">{{item.deviceBrand}}</text>
					</view>
					<view class="action">
						<view class="cu-tag round bg-olive light">在线</view>
					</view>
				</view>
				<!-- <view class="cu-item" :class="'arrow'" @click="gotoDetail">
					<view class="content">
						<text class="cuIcon-tagfill text-red  margin-right-xs"></text>
						<text class="text-grey">NB烟感</text>
					</view>
					<view class="action">
						<view class="cu-tag round bg-orange light">离线</view>
					</view>
				</view> -->
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			deviceList() {
				this.$api.deviceList({shopId:this.shopId}).then(res => {
				   // 获得数据 
					if (res.status === 200) {//登录成功后改变vuex的状态，并退出登录页面
						// this.toMain(res.data.data);
						this.deviceListArr = res.result
					}else {
			...
			gotoDetail(item){
				uni.navigateTo({
					url:  'devicesDetail?detailflag=1&detail=' + JSON.stringify(item)
				})
			},
			gotoAdd(){
				// debugger
				uni.navigateTo({
					url:  'devicesDetail?detailflag=0&shopId=' + this.shopId
				})
			},
			ChooseImage(index) {
				var indextmp = index;
