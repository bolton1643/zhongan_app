	...
	<view class="mosowe-swiper" :style="'height:'+height+'rpx'">
		...
		<!-- #ifndef MP-WEIXIN -->
				<swiper-item
					...
					<!--仅视频展示 -->
					<video class="video" v-if="swiperType === 'video'" :src="item" :controls="false" :muted="true" @error="videoErrorCallback"></video>
					<!-- 图文左右轮播 -->
					...
					<view class="video-type" v-if="swiperType === 'videoType'">
						<image :src="imageKey ? item[imageKey] : item" class="image"></image>
						<image class="image_tips" src="../../static/homepage/ipc_play.png"></image>
					</view>
				</swiper-item>
		<!-- #endif -->
		...
		<!-- 数字指示 -->
		<view class="custom-indicator" :class="indicatorPosition" v-if="indicator === 'number'">
...
export default {
  ...
  data () {
	...
	created () {
		...
		if (this.pyramid && (this.swiperType === 'image' || this.swiperType === 'video'
		|| this.swiperType === 'videoType') && !this.vertical) {
