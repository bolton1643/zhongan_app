	...
	<view class="login-container">
		...
		<view class="container-body">
			...
			<view class="form-btn-group">
				<button :type="primaryType" :disabled="disabled" :loading="loading" class="form-login-btn" @click="loginForm">确认</button>
			</view>
		</view>
		<view class="rules">
	...
	export default {
		...
		methods: {
			...
			checkRePasswordRules(){
				if (!this.form.password) {stopPropagation
			...
			async loginForm() {
				...
				this.$tui.request('/user/resetpassword', 'post', obj).then(res => {
					if (res.success && res.status === 200) {
