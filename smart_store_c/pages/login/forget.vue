<template>
  <view class="login-container">
    <view class="container-title">
      <div class="header-name">忘记密码</div>
    </view>
      ...
      <div class="form-content">
        <div class="form-content-input">
          ...
          <input
            ...
            @blur="focusBlur('mobile')"
            @focus="focusType = 'mobile'"
            placeholder="请输入手机号"
            type="number"
            maxlength="11"
          />
          <uni-icons
            v-if="focusType === 'mobile'"
            @click="clear('mobile')"
            class="input-tip-look"
            type="clear"
            size="22"
            color="#D8D8D8"
          >
        ...
        <div class="form-content-input">
          ...
          <input
            ...
            @focus="focusType = 'captcha'"
            placeholder="请输入验证码"
          />
          <text class="count-down" v-if="!sendCode" @click="getCode"
            >获取验证码</text
          >
        ...
        <div class="form-content-input">
          ...
          <input
            class="input-content password"
            @focus="focusType = 'password'"
            v-model.trim="form.password"
            maxlength="16"
            :password="showPassword"
            @blur="focusBlur('password')"
            placeholder="8-16位字母数字组合"
          />
          <uni-icons
            v-if="focusType === 'password'"
          ...
          <uni-icons
            @click="changePasswordType('showPassword')"
            class="input-tip-look"
            type="eye"
            size="21"
            color="#979797"
        ...
        <div class="form-content-input password-input">
          ...
          <input
            class="input-content password"
            @focus="focusType = 're_password'"
            v-model.trim="form.re_password"
            maxlength="16"
            :password="showRePassword"
            @blur.stop="focusBlur('re_password')"
            placeholder="请再次输入密码"
          />
          <uni-icons
            v-if="focusType === 're_password'"
            @click="clear('re_password')"
            class="input-tip-look"
            type="clear"
            size="22"
            color="#D8D8D8"
          ...
          <uni-icons
            @click="changePasswordType('showRePassword')"
            class="input-tip-look"
            type="eye"
            size="21"
            color="#979797"
  ...
  methods: {
    ...
    checkPasswordRules() {
      ...
      if (!this.form.password) {
        ...
        this.form.password &&
        (this.form.password.length < 8 || this.form.password.length > 16)
      ) {
    ...
    checkRePasswordRules() {
      if (!this.form.password) {
        ...
        this.form.password &&
        this.form.re_password &&
        this.form.password !== this.form.re_password
      ) {
    ...
    getValidCode() {
      ...
      this.$tui
        .request("/public/vCode", "get", obj, false, false, true)
        .then((res) => {
    ...
    async loginForm() {
      ...
      this.$tui
        .request("/user/resetpassword", "post", obj)
        .then((res) => {
          if (res.success && res.status === 200) {
            this.$tui.toast("密码重置成功，快去登录吧");
            setTimeout(() => {
              uni.reLaunch({
                url: "./login",
              });
