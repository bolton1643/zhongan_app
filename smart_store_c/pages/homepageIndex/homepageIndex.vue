		...
		<view class="homepage-index-container">
			<view scroll-y class="top-content">
				...
				<view class="top-operate-content">
					...
					<image src="../../static/homepage/back.png"></image>
			...
			<view class="main-content">
				...
				<title-content title="视频查看" height="418upx" class="video-content">
					<mosowe-swiper swiperType="videoType" indicator="dots" :touchable="true" :height="270" :lists="videoList" pyramid
					 pyramidMargin="48rpx"></mosowe-swiper>
				</title-content>
		...
		<view class="cu-modal show meal-modal" v-if="showModal">
			<view class="cu-dialog">
				...
				<view class="modal-btn-box">
					<button class="primary" type="primary" @click="getMeal">立即续费</button>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.shopInfo = shopList && shopList.length ? shopList[0] : {},
			this.defenceStatus = this.shopInfo.armingStatus ? this.shopInfo.armingStatus === '1' ? true : false : false;
			this.cWidth = uni.upx2px(750);
			this.cHeight = uni.upx2px(420);
			this.getServerData();
		},
		...
		methods: {
			...
			defence() {
				...
				this.$tui.request('/shop/deploy', 'get', obj).then((res) => {
