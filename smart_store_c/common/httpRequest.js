...
const tui = {
  ...
  request: function (url, method, postData, isDelay, isForm, hideLoading) {
    ...
    return new Promise((resolve, reject) => {
      uni.request({
        url: tui.interfaceUrl() + tui.interfaceMainUrl() + url,
        data: postData,
        header: {
  ...
  //获取token
  getToken() {
    return uni.getStorageSync('smart_c_token')
  },
  //判断是否登录
  isLogin: function () {
    return uni.getStorageSync('smart_c_phone') ? true : false
  },
  //跳转页面，校验登录状态
  href(url, isVerify) {
  ...
  handleImageUrl(url) {
    return tui.interfaceUrl() + tui.interfaceMainUrl() + '/public/images/' + url
  }
}
